...
$('body').on('click', '#register-btn', function(){
	$('input, select, #customercode').not('#isbusiness').val(null);
...
$('body').on('click', '.tgl-type', function(){
	
	var type = $(this).attr('storman-type'),
		flds = $(this).attr('storman-flds'),
		mtch = '';
	
	$('.tgl-type').not($(this)).removeClass('bg-4');
	$(this).addClass('bg-4');
	$('#isbuiness').val(type);
	...
	$('.lbl').each(function(index, element) {
	...
	$('.input').each(function(index, element) {
		...
		if($(this).attr(mtch+'-ph'))
			{
			$(this).attr('placeholder', $(this).attr(mtch+'-lbl'));
		...
		if(data.error)
			...
			if(data.accounts)
				...
				$('body').on('click', '.select-code', function(){
					$('.select-code').not($(this)).removeClass('selected-code').removeClass('bg-5');
					$(this).addClass('selected-code').addClass('bg-5');
					});
						...
						'Login': function() {
							...
							$.ajax({
								...
								if(data.success)
									...
									$('#login, #newcomer, #account-type, .pass-row, #confirm-email-row, #can-login').slideUp(350);
									...
									if(data.customer.isbusiness==1)
										{
										$('#tgl-business').click();	
										}
									else
										{
										$('#tgl-residential').click();
										}	
									}
								else if(data.error)
									{
				...
				$('#login, #newcomer, #account-type, .pass-row, #confirm-email-row').slideUp(350);
				$('#customeremail').attr('required', null).attr('readonly', 'readonly');
				$('#profile, #account-tools, #email-row').slideDown(350,0,function(){$('#email-row').removeClass('storman-hdn')});
				$('.register-only, #customeremailc').attr('required', null).attr('disabled', 'disabled');
