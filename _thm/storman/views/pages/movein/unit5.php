<div class="col-xs-12 pad-none">
    ...
    <div class="col-xs-12 pad-none mrg-b-md" id="unit-cats">
        <div class="col-xs-12 mrg-t-lg mrg-b-lg">Start by selecting the unit category.</div>
        <?php
	...
	?>
    <div class="col-xs-12 pad-none mrg-b-md<?php echo $swcls; ?>" id="unit-sizes">
        <div class="col-xs-12 mrg-t-lg mrg-b-lg">Now select the unit sizes you would like to see.</div>
		<?php
        foreach($this->moveins->unittypes as $siz => $size)
            {
			$scls = isset($this->moveins->order['unitsizes']) && $this->moveins->order['unitsizes']==$siz ? 'bg-7' : 'bg-2';
			$shdn = isset($this->moveins->order['unitsizes']) && $this->moveins->order['unitsizes']==$siz ? '' : 'hdn';
        ?>
        <div class="col-xs-12 col-md-4 unit-category-size <?php echo $shdn; ?> unit-category-<?php echo $size['category']; ?>">
            <button type="button" class="inl-button lh-50 <?php echo $scls; ?> bg-4-hover col-xs-12 unit-size" unit-size="<?php echo $siz; ?>">
                <?php echo $size['size']; ?>		
            </button>
        </div>
        <?php		
    ...
    <div class="col-xs-12 pad-none mrg-b-md" id="unit-types">
    	<?php $mhdn = isset($this->moveins->order['unitcode']) ? '' : ' hdn'; ?>
        <div class="col-xs-12<?php echo $mhdn; ?> unit-type-msg">Great. Now you can select a unit that you would like to rent.</div>
        <?php
        ...
        foreach($this->moveins->units as $code => $unit)
			...
			$hdn 	= isset($this->moveins->order['unitsizes']) &&isset($this->moveins->order['unitcats']) && $this->moveins->order['unitcats']==$uc && $this->moveins->order['unitsizes']==$uc.$us ? '' : ' hdn';
