...
<div class="form-group mrg-b-md" id="unit-categories">
	...
	?>
		<option value="<?php echo $cat; ?>" class="unit-cat unit-cat-<?php echo $category;?>"<?php echo $sel; ?>><?php echo $category; ?></option>	
	<?php		
		}
	?>
	</select>
...
<div class="form-group mrg-b-md<?php echo $swcls; ?>" id="unit-sizes">
	...
	foreach($this->moveins->unittypes as $siz => $size)
		{
		$scl = isset($this->moveins->order['unitcats']) && $this->moveins->order['unitcats']==$size['category'] ? '' : ' hdn';
		$sel = isset($this->moveins->order['unitsizes']) && $this->moveins->order['unitsizes']==$siz ? ' selected="selected"' : '';
	?>
		<option value="<?php echo $siz; ?>" class="unit-size unit-size-<?php echo $size['category'].$scl;?>"<?php echo $sel; ?>><?php echo $size['size']; ?></option>	
	<?php		
		}
	?>
	</select>
