...
$('body').on('click', '#account-tglr', function(){
	$('#navbar-main').children('#account-tools').children('#sub-menu').slideToggle(350);
	});
...
$('.input').not('.input-login, .input-reset, .input-search').bind('keydown', function(event){
...
function toolbarDisplay()
	...
	if($('#toolbar').length > 0)
		...
		if($('#form-tabs').length > 0)
			{
			ftop = parseFloat(tpos.top + 30);
			$('#form-tabs').css({'position':'fixed', 'top':tpos.top});		
			$('#form').css('margin-top', parseFloat(tpos.top + $('#form-tabs').height())+'px');
			}
		else
			{
