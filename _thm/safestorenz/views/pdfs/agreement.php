...
.txt-ty, .row div.txt-ty, .group .row div.txt-ty	{font-size:7px;line-height:9px}
.txt-xs, .row div.txt-xs, .group .row div.txt-xs	{font-size:8px;line-height:10px}
.txt-xs, .row div.txt-sm, .group .row div.txt-sm	{font-size:9px;line-height:12px}
.txt-xs, .row div.txt-md, .group .row div.txt-md	{font-size:10px;line-height:14px}
...
.checkbox {border:#ccc solid 1px;height:30px;width:30px;}
...
<div class="row">
    ...
    <div class="wid-40 lh-12 pad-t-md txt-md">
        <?php
        echo $this->moveins->facinfo['facilityfullname'].'<br/>
		'.$this->moveins->facinfo['facilityaddress'].', '.$this->moveins->facinfo['facilitysuburb'].' '.$this->moveins->facinfo['facilitystate'].' '.$this->moveins->facinfo['facilitypostcode'].'<br/>
...
<div class="row">
    ...
    if($this->moveins->customer['isbusiness']==1)
        ...
        <div class="info wid-30"><?php echo $this->moveins->customer['customerfirstname']; ?></div>
        <div class="lbl wid-20">Business Contact</div>
        <div class="info wid-30"><?php echo $this->moveins->customer['contactname']; ?></div>
</div>
<div class="row">
        <div class="lbl wid-20">ABN / ACN</div>
        <div class="info wid-30"><?php echo $this->moveins->customer['companynumber']; ?></div>
        <?php
        }
    else
        {
...
<?php $lbl = $this->moveins->customer['isbusiness']=='1' ? 'Business Owner Details' : 'Alternate Contact Person'; ?>
<h2 class="alg-ctr"><?php echo $lbl; ?></h2>
<div class="row">
...
<div class="row">
    <div class="lbl wid-20">Home</div>
    <div class="info wid-30"><?php echo $this->moveins->customer['customeraltphone']; ?></div>
...
<div class="row">
    ...
    <div class="info wid-20"><?php $this->regioning->currency();echo number_format($this->moveins->order['unitrate'],2,'.',','); ?> per month</div>
    <?php
    ...
    <div class="info wid-30"><?php $this->regioning->currency();echo number_format($t,2,'.',','); ?></div>    
    <?php
		}
	?>
</div>
...
<div class="row">
    ...
    <div class="info wid-20"><?php $this->regioning->currency();echo $this->moveins->facinfo['fees']['cleaning']; ?></div>
    <?php
		}
    ...
    <div class="info wid-40"><?php $this->regioning->currency();echo $this->moveins->facinfo['fees']['admin']; ?></div>
    <?php
		}
	?>
</div>
...
<div class="row">
    ...
    <div class="info wid-20"><?php $this->regioning->currency();echo $this->moveins->facinfo['fees']['late']; ?></div>
...
<div class="group wid-50">
	...
	$w = '200';
	$h = '75';
	if($this->moveins->customer['signsec']!='')
		{
		$w = '200';
		$h = '75';	
		}
    ...
    <div class="row">
        <div class="sign-box wid-50">
            <img src="<?php echo $this->moveins->customer['signature']; ?>" class="signature" alt="" width="<?php echo $w; ?>" height="<?php echo $h; ?>" />
            <?php
			if(isset($this->moveins->customer['dualaccount']) && $this->moveins->customer['dualaccount']=='1' || isset($this->moveins->customer['dualaccount']) && $this->moveins->customer['dualaccount']==true)
				{
			?>
		</div>
    </div>
    <div class="row">
         <div class="sign-box wid-50">
            <img src="<?php echo $this->moveins->customer['signsec']; ?>" class="signature" alt="" width="<?php echo $w; ?>" height="<?php echo $h; ?>" />
            <?php
				}
			?>
        </div>
    ...
    <div class="row">
         <div class="info wid-50">
         Signed by <?php echo $this->moveins->customer['customertitle'].' '.$this->moveins->customer['customerfirstname'].' '.$this->moveins->customer['customersurname']; ?>
         <?php
		if(isset($this->moveins->customer['dualaccount']) && $this->moveins->customer['dualaccount']=='1' || isset($this->moveins->customer['dualaccount']) && $this->moveins->customer['dualaccount']==true)
			{
		?>
		&amp; <?php echo $this->moveins->customer['customeralttitle'].' '.$this->moveins->customer['customeraltfirstname'].' '.$this->moveins->customer['customeraltlastname']; ?>
		<?php
			}
		?>
        </div>
	...
	*/
    ...
    <div class="row">
        <?php $scl = !isset($this->moveins->order['storercheck']) || $this->moveins->order['storercheck']=='' ? ' strikeout' : ''; ?>
...
<div class="group key-group wid-50">
    ...
    <div class="row">
        <h2 class="alg-ctr">Correspondence</h2>
    </div>
    <div class="row">
        <div class="info wid-50">
        	<?php
			if(isset($this->page->postdata['sendnotices']) && $this->page->postdata['sendnotices']=='1')
				{
				echo 'I do want to receive correspondence (including notices) from this facility electronically (including Email or SMS).<br><br>I acknowledge that it is my obligation to update the FO of any changes to my email or phone number.';
				}
			else
				{
				echo 'I do not want to receive correspondence (including notices) from this facility electronically (including Email or SMS)';	
				}
			?>	
        </div>
    </div>
    ...
    <div class="row">
        <h2 class="alg-ctr">Vehicle Details</h2>
    </div>
    <div class="row">
        <div class="lbl wid-20 txt-sm">Make</div> 
        <div class="info wid-30 txt-sm"><?php echo $this->moveins->customer['customercartype']; ?></div>
    </div>
    <div class="row">
        <div class="lbl wid-20 txt-sm">Reg. No</div> 
        <div class="info wid-30 txt-sm"><?php echo $this->moveins->customer['customercarrego']; ?></div>
    </div>
    <div class="row">
        <div class="lbl wid-20 txt-sm">License</div> 
        <div class="info wid-30 txt-sm"><?php echo $this->moveins->customer['customerlicense']; ?></div>
    </div>
    <div class="row">
        <div class="lbl wid-20 txt-sm">Date of Birth</div> 
        <div class="info wid-30 txt-sm"><?php echo $this->moveins->customer['customerdobday'].'-'.$this->moveins->customer['customerdobmonth'].'-'.$this->moveins->customer['customerdobyear']; ?></div>
    </div>
    ...
    <div class="row">
        ...
        <ul class="key-points txt-ty">
            <li>Definitions used in this agreement are set out on this page.</li>
            <li>Goods are stored at your risk and are not insured by the FO.  To the extent permitted by law, the FO is excluded from all liability for the loss of any goods you store in the Facility (clause 12). You should speak to your insurer or insurance broker and ensure you have adequate insurance cover.</li>
            <li>After the Storage Period specified, you or the FO may terminate this agreement (for any reason) on giving the specified days’ notice (clause 21.a).</li>
            <li>You are responsible for any third party you allow onto the Facility (clause 15).</li>
            <li>The only service provided to you, the Storer, by the FO is a licence to use a space provided by the FO for the sole purpose of storing goods.  No other goods or services are provided by the FO (clauses 1.c and 2.a).</li>
            <li>All payments are to be made in advance by you (clause 4.a).</li>
            <li>You must not store hazardous, dangerous, illegal, stolen, perishable, environmentally harmful or explosive goods (clause 5.c).</li>
            <li>You must ensure the goods are dry, clean and free from vermin and food scraps when placed in the Space (clause 5.f).</li>
            <li>The Space is only accessible during set access hours (clause 5.a).</li>
            <li>You must notify the FO of all changes of contact details. If the FO cannot communicate with you, it may communicate with the ACP.</li>
            <li>If you fail to comply with this agreement the FO will have certain rights, including forfeiture of the Deposit and the right to sell/dispose of your goods and deduct from any sale proceeds the amounts you owe to the FO (clause 20).</li>
            <li>The FO has the right to refuse access if all fees are not paid promptly (clause 6).</li>
            <li>The FO has the right to enter the Space in certain circumstances (clauses 17, 18, 19 & 20).</li>
            <li>If you do not remove your goods from the Space on termination, the FO may dispose of your goods and charge you its costs (clause 25).</li>
        </ul>
    </div>
    <?php
	$cs = '';
	
	if(isset($this->moveins->order['insurance']))
		{
		$cs = 'strikeout';	
		}
	
		if(isset($this->moveins->order['insopt']) && $this->moveins->order['insopt']!='')
			{
			//echo '<div class="row"><div class="info wid-50">'.$this->moveins->order['insopt'].'</div></div>';	
		?>
        <div class="row">
            <h2 class="alg-ctr">Insurance</h2>
            <div class="info wid-50"><?php echo $this->moveins->order['insopt']; ?><br>&nbsp;</div>
        </div>
        <?php
			if($this->moveins->order['insoptval']=='1')
				{
		?>
        <div class="row">
            <div class="lbl wid-20 txt-sm">Amount of Coverage</div> 
            <div class="info wid-30 txt-sm">$<?php echo number_format($this->moveins->order['insurance'],2,'.',','); ?></div>
        </div>
        <div class="row">
            <div class="lbl wid-20 txt-sm">Premium</div> 
            <div class="info wid-30 txt-sm">$<?php echo number_format($this->moveins->order['insurrate'],2,'.',','); ?>/ month</div>
        </div>
        <?php
				}
			}
		...
		<div class="row">
        <h2 class="alg-ctr">Insurance</h2>
        <div class="info wid-50 txt-xs">
        1. I accept insurance facilitated by the FO as detailed in the separate insurance agreement numbered:_______________________  The insurance level I have chosen is adequate protection for the value of Goods stored.<br>
        2. I have adequately insured the value of the Goods with my own insurance company or broker who is _____________________________________<br>
        3. I do not accept insurance facilitated by the FO nor do I have the contents insured with any insurance broker or insurance company. I elect to self insure and take the risk of loss or damage to goods stored.<br><br>

		Storer's Signature:______________________________________
        </div>
        </div>
        <?php
			}
	...
	<div class="row">
    	<h2 class="alg-ctr">Insurance</h2>
        <div class="info wid-50">You selected to have additional insurance coverage.<br>&nbsp;</div>
    </div>
    <div class="row">
        <div class="lbl wid-20 txt-sm">Amount of Coverage</div> 
        <div class="info wid-30 txt-sm">$<?php echo number_format($this->moveins->order['insurance'],2,'.',','); ?></div>
