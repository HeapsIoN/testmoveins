...
//tinyMCELoader('facilitycompletereservation');
...
mediaUpload('facilityprivacy', 'facility', 'privacy');

mediaUpload('facilityinsurance', 'facility', 'insurance');

mediaUpload('facilityattachment', 'facility', 'emailfile');
...
function mediaUploadPost(data)
	...
	if(data.result.privacy){$('#privacy-row').html(data.result.privacy).slideDown(350);$('#del-privacy-row').slideDown(350)}
	if(data.result.insurance){$('#insurance-row').html(data.result.insurance).slideDown(350);$('#del-insurance-row').slideDown(350)}
	if(data.result.emailfile){$('#emailfile-row').html(data.result.emailfile).slideDown(350);$('#del-emailfile-row').slideDown(350)}
	}
...
$('body').on('click', '.delete-pdf', function(){
	
	var t = $(this).attr('smp-form'), l = t, ttl = 'Facility', msg = 'facility';
	
	l = l=='emailfile' ? 'email' : l;
	...
	switch(l)
		...
		break;
		case 'privacy':
		ttl = 'Privacy Disclosure Statement';
		msg = 'privacy disclosure statement';
		break;
		case 'insurance':
		ttl = 'Insurance Policy';
		msg = 'insurance policy';
		break;
		}
	
	if($('#dialog').length > 0)
		{
		$('#dialog').html('<p>Are you sure that you want to remove the '+ msg +' PDF for the facility?</p>');	
		}
	else
		{
		$('body').append('<div id="dialog"><p>Are you sure that you want to remove the '+ msg +' PDF for the facility?</p></div>');
		}
	...
	$('#dialog').dialog({
			...
			modal: true,
			dialogClass: 'myDialog',
			title: 'Confirm Facility '+ttl+' PDF Delete',
			buttons: 
				{
				'Yes Delete': function() {
					$('#dialog').dialog('close');
...
function extraToggleChecks()
	{
	if($('#facilityezivisa').val()=='1'){$('#facilityezicardvisa').fadeIn(350);}else{$('#facilityezicardvisa').fadeOut(350);}
	if($('#facilityezimastercard').val()=='1'){$('#facilityezicardmaster').fadeIn(350);}else{$('#facilityezicardmaster').fadeOut(350);}
	if($('#facilityeziamex').val()=='1'){$('#facilityezicardamex').fadeIn(350);}else{$('#facilityezicardamex').fadeOut(350);}
	if($('#facilityezidiners').val()=='1'){$('#facilityezicarddiners').fadeIn(350);}else{$('#facilityezicarddiners').fadeOut(350);}
	}


function extraRadioChecks(opt)
	{
	if($('#facilitypaymenttype').val()==1) // EziDebit
		{
		$('.payby-storpay').slideUp(250);
		$('.payby-ezi').slideDown(250);	
		}
	else if($('#facilitypaymenttype').val()==2) // 
		{
		$('.payby-storpay').slideDown(250);
		$('.payby-ezi').slideUp(250);
		}
	}