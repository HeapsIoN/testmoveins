...
<div class="col-xs-12 lh-50 mrg-t-md" id="list-form">
	<div class="col-xs-12 pad-none bg-lgt-grey txt-drk-grey">
    <label class="src-lbl col-xs-12 col-sm-auto lh-50 txt-drk-grey" for="lookin">Search By</label>
	<?php
	
	$this->page->record['lookin'] = $this->search->lookin;
	
	$this->page->fields->select('', 'lookin', $this->search->options, 'search-input bdr-lgt-grey txt-4 col-xs-12 col-sm-2 pad-none', 'lookin', 'bdr-lgt-grey', '');
	?>
    <label class="src-lbl col-xs-12 col-sm-auto lh-50 txt-drk-grey" for="lookfor">Look For</label>
    <input type="text" name="lookfor" id="lookfor" class="input search-input col-xs-12 col-sm-2 bg-vlg-grey bdr-lgt-grey txt-4" value="<?php echo $this->search->lookfor; ?>" placeholder="Input your search" autofocus="autofocus" />
    <?php
	if(!empty($this->search->filters))
		{
		$lbl = $this->search->filterlbl!='' ? $this->search->filterlbl : 'Filter By'; 
	?>
    <label class="src-lbl col-xs-12 col-sm-auto lh-50 txt-drk-grey" for="filter"><?php echo $lbl; ?></label>
	<?php
	$this->page->record['filter'] = $this->search->filter;
	
	$this->page->fields->select('', 'filter', $this->search->filters, 'search-input bdr-lgt-grey txt-4 col-xs-12 col-sm-2 pad-none', 'filter', 'bdr-lgt-grey', 'Show All');
        }
    ?>
